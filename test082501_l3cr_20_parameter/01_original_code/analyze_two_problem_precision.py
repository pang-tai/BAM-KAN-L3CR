...
def make_audit(synthetic_diag: pd.DataFrame, bam_diag: pd.DataFrame, synthetic: pd.DataFrame, bam: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    gram = pd.read_csv(HERE / "bam_gram_audit.csv")
    ...
    self_tests = pd.read_csv(HERE / "self_tests.csv")
    checks = [
...
def make_figures(primary: pd.DataFrame, thresholds: pd.DataFrame) -> None:
    ...
    fig.savefig(HERE / "gradient_precision_comparison.png", dpi=300)
    fig.savefig(HERE / "gradient_precision_comparison.pdf")
    plt.close(fig)
    ...
    fig.savefig(HERE / "bam_threshold_cost.png", dpi=300)
    fig.savefig(HERE / "bam_threshold_cost.pdf")
    plt.close(fig)
...
def write_tex(summary: pd.DataFrame, thresholds: pd.DataFrame) -> None:
    ...
    (HERE / "l3cr_precision_table.tex").write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_claim_ledger() -> None:
    ...
    pd.DataFrame(rows).to_csv(HERE / "claim_evidence_ledger.csv", index=False)


def main() -> None:
    synthetic = pd.read_csv(HERE / "synthetic_snapshot_metrics.csv")
    bam = pd.read_csv(HERE / "bam_snapshot_metrics.csv")
    synthetic_diag = pd.read_csv(HERE / "synthetic_diagnostics.csv")
    bam_diag = pd.read_csv(HERE / "bam_diagnostics.csv")
    runtime = json.loads((HERE / "runtime.json").read_text(encoding="utf-8"))
    ...
    snapshot_summary.to_csv(HERE / "snapshot_summary.csv", index=False)
    thresholds.to_csv(HERE / "threshold_reach_summary.csv", index=False)
    paired.to_csv(HERE / "paired_comparisons.csv", index=False)
    audit.to_csv(HERE / "algorithm_audit.csv", index=False)
