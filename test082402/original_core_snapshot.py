...
def solve_l3_subproblem(gradient: Tensor, hessian: Tensor, sigma: float, config: SolverConfig) -> dict:
    ...
    for iteration in range(1, pg_limit + 1):
        ...
        if residual_ratio <= config.inner_tolerance:
            converged = True
            break
...
def run_l3cr(initial: Tensor, objective: Objective, config: SolverConfig) -> tuple[Tensor, list[dict]]:
    ...
    hvp_count = 0
    _, gradient_norm = _record(records, "Full-space L3CR", 0, started, objective, vector, gradient_equivalents, hvp_count)
    dimension = vector.numel()
    for iteration in range(1, config.max_updates + 1):
        projected_cost = gradient_equivalents + dimension + 1
        if time.perf_counter() - started >= config.time_limit or projected_cost > config.max_gradient_equivalents:
            break
        ...
        _, gradient_norm = _record(
            ...
            inner_residual_ratio=solved["residual_ratio"], inner_converged=float(solved["converged"]),
...
def self_tests() -> list[dict]:
    ...
    rows.append({"test": "inner_optimality", "value": solved["residual_ratio"], "passed": solved["residual_ratio"] <= 1.0e-12})
    return rows
