...
ROOT = HERE.parent
OUTPUT_DIR = Path(os.environ.get("BAMKAN_20P_OUT_DIR", str(HERE))).resolve()
...
def run_synthetic() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for scale, condition in ((1.0e-2, "near_local_solution"), (0.2, "far_negative_control")):
        for seed in FORMAL_SEEDS:
            ...
            pd.DataFrame(metric_rows).to_csv(OUTPUT_DIR / "synthetic_snapshot_metrics_progress.csv", index=False)
            pd.DataFrame(diagnostics).fillna(-1.0).to_csv(OUTPUT_DIR / "synthetic_diagnostics_progress.csv", index=False)
...
def run_bam() -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    for seed in BAM_SEEDS:
        ...
        pd.DataFrame(metric_rows).to_csv(OUTPUT_DIR / "bam_snapshot_metrics_progress.csv", index=False)
        pd.DataFrame(diagnostic_rows_all).fillna(-1.0).to_csv(OUTPUT_DIR / "bam_diagnostics_progress.csv", index=False)
...
def main() -> None:
    ...
    tests.to_csv(OUTPUT_DIR / "self_tests.csv", index=False)
    ...
    synthetic_metrics.to_csv(OUTPUT_DIR / "synthetic_snapshot_metrics.csv", index=False)
    synthetic_diagnostics.to_csv(OUTPUT_DIR / "synthetic_diagnostics.csv", index=False)
    bam_metrics, bam_diagnostics, bam_audit, bam_samples = run_bam()
    bam_metrics.to_csv(OUTPUT_DIR / "bam_snapshot_metrics.csv", index=False)
    bam_diagnostics.to_csv(OUTPUT_DIR / "bam_diagnostics.csv", index=False)
    bam_audit.to_csv(OUTPUT_DIR / "bam_gram_audit.csv", index=False)
    bam_samples.to_csv(OUTPUT_DIR / "bam_test_metrics_per_sample.csv", index=False)
